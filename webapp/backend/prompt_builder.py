...
def build_system_prompt(language: str = "en") -> str:
    ...
    if language == "zh":
        return (
            ...
            "专利检索排序→专利抽取→X-Y 综合→风险筛查→2-3轮DOE设计（筛选→优化→确认）→"
            "后实验学习→组合复制。\n"
            ...
            "## DOE 设计规则（核心！）\n"
            "基于专利证据设计 2-3 轮 DOE，而非一轮了事：\n"
            "- **第1轮 · 因子筛选 (Screening)**：从专利中提取的所有关键 X 变量中，"
            "用部分因子设计（如 2^(k-p)，Resolution≥IV）筛选出真正影响 CTQ 的主效应。"
            "目标：从 5-10 个候选 X 缩小到 2-4 个 dominant levers\n"
            "- **第2轮 · 响应面优化 (Optimization)**：对第1轮筛选出的关键 X，"
            "用中心复合设计 (CCD) 或 Box-Behnken 找到最优参数组合。"
            "目标：确定最佳配方/工艺窗口\n"
            "- **第3轮 · 确认实验 (Confirmation，可选)**：对优化出的最优配方，"
            "跑 3-5 次重复确认实验，验证模型预测的准确性和长期稳定性\n"
            "- 每轮实验必须写清楚：Hypothesis、Factors（含Levels/Range）、"
            "Responses（含Test Method/Target）、Controls、Statistical Design、"
            "Decision Rule（Go/No-Go 标准）\n"
            "- 不同轮次尝试不同的 X 组合——不要第2轮只是重复第1轮\n\n"
            "## 关键规则\n"
        ...
        return (
            ...
            "patent search and ranking→patent extraction→X-Y synthesis→risk screening→"
            "2-3 round DOE design (screening→optimization→confirmation)→"
            ...
            "## DOE Design Rules (CRITICAL!)\n"
            "Design 2-3 rounds of DOE based on patent evidence — never stop at one round:\n"
            "- **Round 1 · Factor Screening**: From all critical X variables extracted from patents, "
            "use fractional factorial design (e.g. 2^(k-p), Resolution≥IV) to identify the 2-4 "
            "dominant levers that actually drive CTQs. "
            "Goal: narrow 5-10 candidate Xs → 2-4 dominant levers\n"
            "- **Round 2 · Response Surface Optimization**: For the key Xs from Round 1, "
            "use Central Composite Design (CCD) or Box-Behnken to find the optimal parameter combination. "
            "Goal: identify the best formulation/process window\n"
            "- **Round 3 · Confirmation Run (optional but recommended)**: Run 3-5 replicate "
            "confirmation experiments at the optimal point to verify model predictions "
            "and long-term stability\n"
            "- Each round MUST specify: Hypothesis, Factors (with Levels/Range), "
            "Responses (with Test Method/Target), Controls, Statistical Design, "
            "Decision Rule (Go/No-Go criteria)\n"
            "- Try DIFFERENT X combinations across rounds — Round 2 should not just repeat Round 1\n\n"
            "## Key Rules\n"
