...
> ⚠️ **人在回路中（Human in the Loop）。** FORGE AI 是决策支持引擎——不是决策者。它发现证据、提取关系、推荐实验。但有经验的工程师仍然需要判断哪些专利重要、优先测试哪些 X 变量、DOE 方案是否在产线上可行。AI 加速苦活。化学家掌握判断。
...
> 💡 **输入决定输出。** 一个模糊的 VOC——*"我需要更好的涂层"*——只会产生模糊的结果。一个精准的 VOC——*"我需要一款 UV 固化聚碳酸酯硬质涂层，铅笔硬度 9H，550nm 透光率 >90%，1000 次 Taber 磨耗后雾度 <2%"*——给了 AI 可以推理的具体目标。说出你的材料、目标性能（带数值）、约束条件和应用场景。输入越多，输出越好。
...
```
  ...
  │       (专利搜索、X-Y 提取、空白区域分析)                          │
  │                                                                 │
  │  ② 运行实验 ──▶ SPC Platform ──▶ 统计分析                        │
  │       (主效应、响应面、意愿函数优化)                               │
  │                                                                 │
  │  ③ SPC 结果 ──▶ FORGE AI ──▶ 第 2 轮 DOE                        │
  │       (AI 解读你的数据，优化模型)                                  │
  │                                                                 │
  │  ④ 循环。知识跨项目累积。                                         │
  │                                                                 │
...
| 仓库 | 角色 | 技术 |
|------|------|------|
| **[FORGE AI](https://github.com/newbison/AI_CHEMIST)** | VOC → 专利情报 → 初始 DOE → 报告 | DeepSeek + FastAPI + React |
| **[SPC Platform](https://github.com/newbison/spc-batch-monitor)** | SPC 图表 → DOE 分析 → 意愿优化 → 结果 | Streamlit + Plotly + NumPy/SciPy |

> **连接方式**：FORGE AI 告诉你*该测什么、为什么*。你做实验。SPC Platform 告诉你*发生了什么*——主效应、响应面、过程能力指数、意愿分数。把结果反馈给 FORGE AI，它设计下一轮实验。这就是把文献综述变成学习系统的闭环。
